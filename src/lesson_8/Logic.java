...
//        printMap();
        if (checkWinLines(DOT_X, DOTS_TO_WIN)) {
            new GameWindow("Вы выиграли!!!");
//            System.out.println("Вы выиграли!!!");
            return;
        }
        if (isFull()) {
            new GameWindow("Ничья!");
...
//        printMap();
        if (checkWinLines(DOT_O, DOTS_TO_WIN)) {
            new GameWindow("Комьютер победил");
    ...
    static void aiTurn() {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (isCellValid(i, j)) {
                    map[i][j] = DOT_O;
//                    battleMap.drawO(Graphics g, i, j);
                    if (checkWinLines(DOT_O, DOTS_TO_WIN)) {
                        return;
                    }
                    map[i][j] = DOT_EMPTY;
...
//    }
    ...
    static boolean checkLine(int cy, int cx, int vy, int vx, char dot, int dotsToWin) {
        ...
        if (dotsToWin==DOTS_TO_WIN){
            START_X=cx;
            START_Y=cy;
            END_X=cx+vx*(dotsToWin-1);
            END_Y=cy+vy*(dotsToWin-1);
            VX=vx;
            VY=vy;
        }
            return true;
